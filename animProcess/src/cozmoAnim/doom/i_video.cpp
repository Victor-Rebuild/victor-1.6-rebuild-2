...
{
	sf::Uint8 colouredPixels[SCREENHEIGHT*SCREENWIDTH * 3] = { 0 };
	for (int i = 0; i < SCREENHEIGHT*SCREENWIDTH; i++)
	{
		colouredPixels[i * 3] = colors[screens[0][i]].r;
		colouredPixels[i * 3 + 1] = colors[screens[0][i]].g;
		colouredPixels[i * 3 + 2] = colors[screens[0][i]].b;
	}
...
void I_InitGraphics(void)
{
	...
	//window->create(X_width, X_height);
...
//  else
//  {
    window->create(_gDoomPortWidth, _gDoomPortHeight);//sf::VideoMode::getDesktopMode(), "SFML-DOOM", sf::Style::Fullscreen);
		isFullscreen = true;
