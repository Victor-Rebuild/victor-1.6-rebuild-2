...
unsigned int _gDoomPortWidth;
unsigned int _gDoomPortHeight;
bool _gDoomPortRun = true;
bool _gMainLoopStarted = false;
std::string _gDoomResourcePath;
...
void IdentifyVersion (void)
{
    ...
    PRINT_NAMED_WARNING("DOOM", "Looking for %s", doom2wad.c_str());
    file.open(doom2wad);
    if (file.good())
    {
    ...
    PRINT_NAMED_WARNING("DOOM", "Looking for %s", plutoniawad.c_str());
    ...
    PRINT_NAMED_WARNING("DOOM", "Looking for %s", tntwad.c_str());
    file.open(tntwad);
    if (file.good())
    {
        Game::gamemode = GameMode_t::commercial;
    ...
    PRINT_NAMED_WARNING("DOOM", "Looking for %s", doomuwad.c_str());
    file.open(doomuwad);
    if (file.good())
    {
    ...
    PRINT_NAMED_WARNING("DOOM", "Looking for %s", doomwad.c_str());
    ...
    PRINT_NAMED_WARNING("DOOM", "Looking for %s", doom1wad.c_str());
...
void D_DoomMain (void)
{
  ...
  // this also sets some flags we need, so don't delete
    D_CheckNetGame ();
