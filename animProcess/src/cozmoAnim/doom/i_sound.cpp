 ...
 void I_Sound::initialise()
{
    ...
    printferror( "I_InitSound: sound module ready\n");
}
...
void I_Sound::startSound(void * origin_p, int sfx_id, int volume)
{
    ...
    if (soundBuffers.find(sfx->name) == soundBuffers.end())
    {
        ...
        if (!soundBuffers[sfx->name].loadFromSamples(newData, 1, SAMPLERATE))
            printferror( "Failed to load sound");
    }

    sounds.emplace_back( sf::Sound(), origin);// push_back(std::make_pair( sf::Sound() , origin));
...
void I_Sound::playMusic(const std::string& songname, bool looping)
{
  ...
  PRINT_NAMED_WARNING("DOOM","About to load %d", length);

    doomMusic = Timidity_LoadSongMem(midiConversionBuffer, length);

  PRINT_NAMED_WARNING("DOOM", "which is %d", MIDI_CHANNELS * MIDI_FORMAT_BYTES * doomMusic->samples);
    ...
    if (doomMusic)
    {
        ...
        musicSoundBuffer->loadFromSamples(newData,MIDI_CHANNELS,MIDI_RATE);
