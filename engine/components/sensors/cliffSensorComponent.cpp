...
namespace {
  ...
  bool blackDeskCompatMode = Util::FileUtils::FileExists("/data/data/rebuild/blackDeskCompatMode");
  
  // IIR filter coefficient used for filtering raw cliff sensor data
  const f32 kCliffFiltCoef = blackDeskCompatMode ? 0.8f : 0.70f;
  ...
  const u16 kCliffDetectAllowedDeltaDefault = blackDeskCompatMode ? 40 : 25;
  ...
  const u16 kCliffDetectAllowedDeltaLow = blackDeskCompatMode ? 25 :  15;
  ...
  const u16 kCliffValDarkSurface = blackDeskCompatMode ? 230 : 280;
